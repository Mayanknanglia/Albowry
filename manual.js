...
function populateManualWorkerDD(){
  ...
  if(ind.length){h+='<optgroup label="🇮🇳 Indian">';ind.forEach(x=>h+=`<option value="${x.wid}">${x.name} (${x.prof})</option>`);h+='</optgroup>';}
  if(pak.length){h+='<optgroup label="🇵🇰 Pakistani">';pak.forEach(x=>h+=`<option value="${x.wid}">${x.name}</option>`);h+='</optgroup>';}
...
async function manualQuickCheckIn(){
  ...
  const shift=document.getElementById('manualQuickShift').value;
  ...
  if(existing){
    if(existing.status==='checked_in')return toast('Already in!','err');
    if(existing.status==='completed')return toast('Done','err');
    if(existing.status==='pending_checkin'){const u={...existing};u.checkinTime=existing.checkinReqTime;u.shift=shift;u.status='checked_in';await FB.save(COL.A,existing.id,u);toast('✅ Approved!');return;}
  }
  const now=new Date().toISOString(),recId='att_'+Date.now()+'_'+wid;
  await FB.save(COL.A,recId,{recId,wid,name:worker.name,prof:worker.prof,sec:worker.sec,shift:shift,date:today,checkinReqTime:now,checkinTime:now,checkoutReqTime:null,checkoutTime:null,total:0,regular:0,compOT:0,extraOT:0,ot:0,status:'checked_in'});
  toast('✅ '+worker.name+' → '+shift+' shift!');
...
async function manualQuickCheckOut(){
  ...
  if(rec.status==='pending_checkout'){const u={...rec};u.checkoutTime=rec.checkoutReqTime;const c=calcHours(u.checkinTime,u.checkoutTime);u.total=c.total;u.regular=c.regular;u.compOT=c.compOT;u.extraOT=c.extraOT;u.ot=c.ot;u.status='completed';await FB.save(COL.A,rec.id,u);toast('✅ '+u.total.toFixed(2)+'h');return;}
  ...
  await FB.save(COL.A,rec.id,u);toast('✅ '+worker.name+' out! '+u.total.toFixed(2)+'h');
  document.getElementById('manualWorker').value='';
}

async function manualCustomCheckIn(){
  const wid=document.getElementById('manualCustomWorker').value,date=document.getElementById('manualDate').value,time=document.getElementById('manualCheckInTime').value,shift=document.getElementById('manualCustomShift').value;
  if(!wid)return toast('Select worker','err');if(!date)return toast('Date','err');if(!time)return toast('Time','err');if(!shift)return toast('Select shift','err');
  const worker=gW().find(x=>x.wid===wid),existing=gA().find(a=>a.wid===wid&&a.date===date);
  if(existing&&(existing.status==='checked_in'||existing.status==='completed'))return toast('Already exists','err');
  const now=new Date(date+'T'+time+':00').toISOString(),recId='att_'+Date.now()+'_'+wid;
  await FB.save(COL.A,recId,{recId,wid,name:worker.name,prof:worker.prof,sec:worker.sec,shift:shift,date,checkinReqTime:now,checkinTime:now,checkoutReqTime:null,checkoutTime:null,total:0,regular:0,compOT:0,extraOT:0,ot:0,status:'checked_in'});
  toast('✅ '+worker.name+' → '+shift+' shift on '+date);
}
...
async function manualCustomCheckOut(){
  ...
  if(!rec)return toast('No check-in','err');if(rec.status==='completed')return toast('Done','err');
...
async function bulkCheckInAll(){
  ...
  if(!shift)return toast('Select shift','err');
  let workers=gW().filter(w=>w.on);if(sec)workers=workers.filter(w=>w.sec===sec);
  const today=tD(),todayAtt=gA().filter(a=>a.date===today);
  const notCheckedIn=workers.filter(w=>!todayAtt.find(a=>a.wid===w.wid));
  if(!notCheckedIn.length)return toast('All in!','info');
  confirmDlg('Bulk Check-In?',`${notCheckedIn.length} → ${shift} shift?`,async()=>{
    const now=new Date().toISOString();
    for(const w of notCheckedIn){
      const recId='att_'+Date.now()+'_'+w.wid+Math.random();
      await FB.save(COL.A,recId,{recId,wid:w.wid,name:w.name,prof:w.prof,sec:w.sec,shift:shift,date:today,checkinReqTime:now,checkinTime:now,checkoutReqTime:null,checkoutTime:null,total:0,regular:0,compOT:0,extraOT:0,ot:0,status:'checked_in'});
    }
    toast('✅ '+notCheckedIn.length+' in '+shift+' shift!');
  });
}
...
async function bulkCheckOutAll(){
  ...
  if(!active.length)return toast('None','info');
  confirmDlg('Bulk Check-Out?',`${active.length}?`,async()=>{
    const now=new Date().toISOString();
...
function loadManualToday(){
  ...
  const dayWorking=working.filter(a=>a.shift==='Day'||!a.shift);
  const nightWorking=working.filter(a=>a.shift==='Night');
  const countEl=document.getElementById('manualStatusCount');
  if(countEl)countEl.innerHTML=`<span class="tag tag-r" style="margin-right:6px">❌ ${notCheckedIn.length}</span><span class="tag tag-o" style="margin-right:6px">⏰ ${pending.length}</span><span class="tag tag-b" style="margin-right:6px">☀️ ${dayWorking.length}</span><span class="tag tag-o" style="margin-right:6px">🌙 ${nightWorking.length}</span><span class="tag tag-g">✅ ${completed.length}</span>`;
  let html='';
  if(notCheckedIn.length){html+=`<h4 style="margin:16px 0 10px;color:var(--r);font-weight:700">❌ Not Checked In (${notCheckedIn.length})</h4><div class="t-wrap" style="margin-bottom:20px"><table><thead><tr><th>#</th><th>Name</th><th>Work</th><th>Country</th><th>Action</th></tr></thead><tbody>${notCheckedIn.map((w,i)=>`<tr><td>${i+1}</td><td><b>${w.name}</b></td><td>${w.prof||'-'}</td><td><span class="tag tag-${w.sec==='Indian'?'ind':'pak'}">${w.sec==='Indian'?'🇮🇳':'🇵🇰'}</span></td><td><button class="btn btn-success btn-sm" onclick="manualSingleCheckIn('${w.wid}','Day')">☀️ Day</button> <button class="btn btn-outline btn-sm" onclick="manualSingleCheckIn('${w.wid}','Night')">🌙 Night</button></td></tr>`).join('')}</tbody></table></div>`;}
  if(working.length){html+=`<h4 style="margin:16px 0 10px;color:var(--p);font-weight:700">🟢 Working (${working.length})</h4><div class="t-wrap" style="margin-bottom:20px"><table><thead><tr><th>#</th><th>Name</th><th>Shift</th><th>In</th><th>Action</th></tr></thead><tbody>${working.map((a,i)=>`<tr><td>${i+1}</td><td><b>${a.name}</b></td><td><span class="tag ${a.shift==='Night'?'tag-o':'tag-b'}">${a.shift==='Night'?'🌙 Night':'☀️ Day'}</span></td><td style="color:#059669">${fT(a.checkinTime)}</td><td><button class="btn btn-danger btn-sm" onclick="manualSingleCheckOut('${a.id}')">🔒 Out</button></td></tr>`).join('')}</tbody></table></div>`;}
  if(pending.length){html+=`<h4 style="margin:16px 0 10px;color:var(--o);font-weight:700">⏰ Pending (${pending.length})</h4><div class="t-wrap" style="margin-bottom:20px"><table><thead><tr><th>#</th><th>Name</th><th>Type</th><th>Shift</th><th>Time</th><th>Action</th></tr></thead><tbody>${pending.map((a,i)=>{const isIn=a.status==='pending_checkin';return`<tr><td>${i+1}</td><td><b>${a.name}</b></td><td><span class="tag ${isIn?'tag-g':'tag-r'}">${isIn?'IN':'OUT'}</span></td><td><span class="tag ${a.shift==='Night'?'tag-o':'tag-b'}">${a.shift==='Night'?'🌙':'☀️'}</span></td><td>${fT(isIn?a.checkinReqTime:a.checkoutReqTime)}</td><td><button class="btn btn-success btn-sm" onclick="doApprove('${a.id}')">✅</button> <button class="btn btn-danger btn-sm" onclick="doReject('${a.id}')">❌</button></td></tr>`;}).join('')}</tbody></table></div>`;}
  if(completed.length){html+=`<h4 style="margin:16px 0 10px;color:var(--g);font-weight:700">✅ Completed (${completed.length})</h4><div class="t-wrap"><table><thead><tr><th>#</th><th>Name</th><th>Shift</th><th>In</th><th>Out</th><th>Total</th><th>OT</th><th>Action</th></tr></thead><tbody>${completed.map((a,i)=>`<tr><td>${i+1}</td><td><b>${a.name}</b></td><td><span class="tag ${a.shift==='Night'?'tag-o':'tag-b'}">${a.shift==='Night'?'🌙':'☀️'}</span></td><td style="color:#059669">${fT(a.checkinTime)}</td><td style="color:#dc2626">${fT(a.checkoutTime)}</td><td style="color:var(--p);font-weight:700">${a.total.toFixed(2)}h</td><td style="color:#d97706">${a.ot.toFixed(2)}h</td><td><button class="btn btn-outline btn-sm" onclick="undoCO('${a.id}')">↩️</button> <button class="btn btn-danger btn-sm" onclick="undoCI('${a.id}')">🗑️</button></td></tr>`).join('')}</tbody></table></div>`;}
...
async function manualSingleCheckIn(wid,shift){
  ...
  await FB.save(COL.A,recId,{recId,wid,name:worker.name,prof:worker.prof,sec:worker.sec,shift:shift||'Day',date:today,checkinReqTime:now,checkinTime:now,checkoutReqTime:null,checkoutTime:null,total:0,regular:0,compOT:0,extraOT:0,ot:0,status:'checked_in'});
  toast('✅ '+worker.name+' → '+(shift||'Day')+' shift!');
}
